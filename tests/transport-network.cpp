...
BOOST_AUTO_TEST_CASE(basic)
{
    ...
    using Event = TransportNetwork::PassengerEvent;
    ok = nw.RecordPassengerEvent(station0.id, Event::In);
    BOOST_REQUIRE(ok);
    ...
    ok = nw.RecordPassengerEvent(station1.id, Event::In);
    BOOST_REQUIRE(ok);
    BOOST_CHECK_EQUAL(nw.GetPassengerCount(station0.id), 2);
    BOOST_CHECK_EQUAL(nw.GetPassengerCount(station1.id), 1);
    BOOST_CHECK_EQUAL(nw.GetPassengerCount(station2.id), 0);
    ok = nw.RecordPassengerEvent(station0.id, Event::Out);
    BOOST_REQUIRE(ok);
    BOOST_CHECK_EQUAL(nw.GetPassengerCount(station0.id), 1);
    ok = nw.RecordPassengerEvent(station2.id, Event::Out); // Negative count
    BOOST_REQUIRE(ok);
